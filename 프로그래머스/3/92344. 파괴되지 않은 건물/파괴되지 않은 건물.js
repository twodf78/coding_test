...
const solution = (board, skill) => {
    const pSum = Array.from({length: board.length+1}, _ =>Array.from({length: board[0].length+1}, _ =>0) );
    ...
    return board.map((v)=>v.filter((v2)=>v2>0).length).reduce((acc,cur)=>acc+cur, 0);
}